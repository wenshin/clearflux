import {customizeDeepClone} from './clone';
import {
  DEFAULT_SEPARATOR, flatObj, iterateObj,
  putFlatedObj, putEntireFlatedObj, patchFlatedObj
} from './flatobj';
...
export default class FlatMap {
   ...
   * @param  {Object}  shallowClonePropSet [可选]，当`isDeepClone = true`时，指定哪些 query 浅拷贝
   *                                       这在一些比较大的数据深拷贝时有用
   * @param  {Boolean} isDeepClone         默认 true，如果为 false 将进行浅拷贝
   * @return {All}
   */
  get(query, shallowClonePropSet, isDeepClone=true) {
    // 如果第二个参数为 false 则认为是设置 isDeepClone 为 false
  ...
  isIn(query) {
    let props = query.split(this.separator);
    let prop = props.pop();
    let parentQuery = props.join(this.separator);
    let parent = this.get(parentQuery, false);
    return typeof parent === 'object' && (prop in parent);
  }
}
