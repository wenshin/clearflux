 ...
 * @param  {Function} compare   比较函数，参数为[key(当前遍历的 Key), target, iterObj, queries]
 ...
 * 如：
 * ```
 * > flateObj({a: 1, c: {b: 1, d: 2}})
 * {a: 1, c: {b: 1, d: 2}, 'c.b': 1, 'c.d': 2}
 * ```
 * @param  {PlainObject} obj       必须是 plain object
 * @param  {String}      prefix    展开时添加前缀
 * @param  {String}      separator 展开属性名的分隔符
 * @return {Object}                展开的对象
 */
 ...
 *     接受以下3个参数`Function(query, newValue[, isChangeSource])`。
 *       `query` 是当前的对象属性查询字符串，如：'a.b.c'；
 *       `newValue` 是更新的值；
 *       `isChangeSource` 可选的，如果为`true`说明是引起变更的最底层属性，
 *         如：更新`{a: {b: 2, c: {d: 3}}}`到`{a: {b: 1}}`中的 'a.b'和 'a.c'；
...
export function patchFlatedObj(srcFlated, updated, handleChanged, separator, prefix) {
  ...
  iterateObj(srcFlated, updated, (key, target, iterObj, queries) => {
    ...
    let query = getArrayLast(queries);
    let next = true;
    if (targ !== iter) {
      if ( _isPlainObject(targ) && _isPlainObject(iter) ) {
        ...
        target[key] = iter;
        srcFlated[query] = target[key];
        handleChanged(query, target[key]);
