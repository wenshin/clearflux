 ...
 */
export function pipelineWrap (input) {
  let _output = input;
  let _pipeCount = 0;
  let _blocking = false;
  ...
  function _execSyncPipe(pipeInput, {name, pipe, preMixins=[], postMixins=[]}) {
    let pre = gExecMixins(preMixins, {value: pipeInput, name, pipe});
    let output = pre.pipe(pre.value);
    let post = gExecMixins(postMixins, {...pre, value: output});
    return post.value;
  }
  ...
  function _execAsyncPipe() {
    ...
    let pre = gExecMixins(preMixins, {value: _output, name, pipe});
    ...
    promise
      .then(data => {
        let post = gExecMixins(postMixins, {...pre, value: data});

        _output = post.value;
        _blocking = false;

        for (let syncPipeObj of _syncPipesAfterAsync.get(pipeObj)) {
          _output = _execAllSyncPipe(_output, syncPipeObj);
        }
 ...
 */
function gExecMixins(mixins, mixinArg) {
