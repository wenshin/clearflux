...
export default class Store extends EventEmitter {
  ...
  constructor(data, shallowCloneProps) {
    ...
    this._innerWriter = {name: 'combineLoadings'};
  ...
  combine(config) {
    ...
    let getHandleChange = (prop, get) => (...args) => {
      ...
      args.map(v => {
        ...
        combineErrors.concat(v.errors);
        combineLoading = combineLoading || v.loading;
      });

      this.put(prop, get.apply(null, combineValues), this._innerWriter);

      if (combineLoading) this.startLoading(prop, this._innerWriter);
      else this.stopLoading(prop, this._innerWriter);
    ...
    for (let prop of props) {
      ...
      if (this._data.isIn(prop)) {
        throw new Error(`Query "${prop}" existed`);
      }
      if (!(get && get instanceof Function)) {
        throw new Error(`Config "get" should be a function`);
      }
