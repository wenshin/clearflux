...
export default class Store extends EventEmitter {
   ...
   * @param  {Array|String}   queriesStr 监视的参数
   * @param  {Function}       callback   回调函数
   * @return {undefined}
   */
  onChange(queriesStr, callback) {
    let queries;
    if ( queriesStr instanceof Array ) {
      queries = queriesStr;
      queriesStr = queries.join(QUERY_SEPARATOR);
    } else if ( typeof queriesStr === 'function' ) {
      callback = queriesStr;
      queriesStr = Store.ALL;
    } else {
      queries = queriesStr.split(QUERY_SEPARATOR);
    }
  ...
  offChange(queriesStr, callback) {
    if (queriesStr instanceof Array) queriesStr = queriesStr.join(QUERY_SEPARATOR);
    this.off(CHANGE_EVENT, queriesStr, callback);
  }
