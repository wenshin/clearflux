...
export default class Store extends EventEmitter {
  ...
  constructor(data, shallowCloneProps) {
    ...
    this._innerWriter = {name: '__inner__'};
    this.registerWriter(this._innerWriter);
  ...
  patch(value, writer) {
    this.permit(writer);
    this._data.patch(value, (q, v, isChangeSource) => {
      // 如果是引起更新的元素，则重置 loading 和 errors 信息
      if (isChangeSource) this.resetStatus(q);
      this.trigger(CHANGE_EVENT, q);
    });
  }
  ...
  setErrors(query, errors, writer) {
    this.needQuery(query);
    this.permit(writer);
    this.stopLoading(query, this._innerWriter);
  ...
  startLoading(query, writer) {
    this.needQuery(query);
    if (this._loadingQueries.has(query)) return;
    this.permit(writer);
  ...
  resetStatus(query) {
    this.stopLoading(query, this._innerWriter);
    this.removeErrors(query, this._innerWriter);
  }
  ...
  combine(config) {
    let getHandleChange = (prop, get) => (...args) => {
      ...
      this.put(prop, get.apply(null, combineValues), this._innerWriter);

      combineLoading
        ? this.startLoading(prop, this._innerWriter)
        : this.stopLoading(prop, this._innerWriter);

      combineErrors.length
        ? this.setErrors(prop, combineErrors, this._innerWriter)
        : this.removeErrors(prop, this._innerWriter);
    };
  ...
  combineLoadings(prop, configs) {
    this.combineStores(prop, configs, storeMap => {
      ...
      if (loading && !this._loadingQueries.has(prop)) this.startLoading(prop, this._innerWriter);
      if (!loading && this._loadingQueries.has(prop)) this.stopLoading(prop, this._innerWriter);
    });
  }
