...
export default class Store extends EventEmitter {
  ...
  get(queries=[], isDeepClone=true) {
    ...
    for (let q of queries) {
      values.push({
        value: this._data.get(q, this._shallowClonePropSet, isDeepClone),
        loading: this._loadingQueries.has(q),
        errors: this._queryErrors[q]
      });
    }
    return values;
  }
  ...
  bind(type, queriesStr, callback) {
    ...
    let wrapperCb = () => callback.apply(null, this.get(queries));
